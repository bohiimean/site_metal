...
class ColorPalette(models.Model):
    """Именованный набор цветов (например «Нержавейка 304, полировка»).

    Живой источник цветов для обработок товаров: на узле-обработке дерева
    опций (catalog.ProductOptionNode.palette) можно выбрать палитру — тогда
    доступные цвета берутся из неё живьём. Убрали цвет из палитры или
    выключили её — он пропадёт у всех обработок, ссылающихся на палитру.
    Привязка простая, без глобальных правил и цепочек разрешения: что выбрано
    на самом узле, то и действует; узлу всегда можно вернуть ручной набор
    цветов (режим-исключение).
    """
    name = models.CharField('Название', max_length=120)
    colors = models.ManyToManyField(
        Color, related_name='palettes',
        verbose_name='Цвета',
        help_text='Набор оттенков. Меняется здесь — сразу отражается на всех '
                  'обработках товаров, где выбрана эта палитра.',
    )
    is_active = models.BooleanField('Активна', default=True)
